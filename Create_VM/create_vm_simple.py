...
def CheckVMAvailable(connection, NEW_VM_NAME):
    ...
    if check_vm == 1:
        print (f"SUCCESS: Виртуальная машина {NEW_VM_NAME} создана")
    else:
        print (f"ERROR: Виртуальная машина {NEW_VM_NAME} НЕ создана.")
...
def main():
    ...
    for i in range(1, VM_NUM):
        ...
        #print (f"Ждем создания диска ВМ {NEW_VM_NAME}")
    ...
    if START_VM == 'y':
        ...
        for i in range(1, VM_NUM):
            ...
            print (f"INFO: Ждем разблокирования диска ВМ {NEW_VM_NAME}")
            CheckVMdisk(connection, NEW_VM_NAME)
            StartVM(connection,NEW_VM_NAME)
            print (f"Ждем запуска ВМ {NEW_VM_NAME}")
